...
INPUT_DIR = "results/dsccl"

# Output directory for statistics
OUTPUT_DIR = "stats/dsccl"
...
def process_all_json_files():
    ...
    for json_file in sorted(json_files):
        ...
        try:
            ...
            # Extract metadata
            mpi_impl = data.get("mpi_implementation") or data.get("implementation") or "unknown"
    ...
    if all_results:
        ...
        #            if k != "per_rank_means_us" and k != "dtype"}
        #     cleaned_results.append(row)

        # # Use the same column list as the set of row headers
        # row_headers = csv_columns

        # # Write transposed CSV
        # with open(csv_path, 'w', newline='') as csvfile:
        #     writer = csv.writer(csvfile)

        #     # First row: run labels
        #     header = ["metric"] + [f"run_{i+1}" for i in range(len(cleaned_results))]
        #     writer.writerow(header)

        #     # Each metric becomes a row
        #     for metric in row_headers:
        #         row = [metric]
